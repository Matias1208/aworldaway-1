...
// --- Types and mock data (you can replace with your store) ---
...
const mockRows: ExoplanetRow[] = Array.from({ length: 100 }, (_, i) => {
  ...
  return {
    id: `KOI-${1000 + i}`,
    classification:
      score < 0.33 ? "False positive" : score > 0.66 ? "Confirmed" : "Candidate",
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-cyan-200">Results table (first 50)</h3>
            <p className="text-slate-400 text-sm">Detailed view by object</p>
          </div>
        </div>
        ...
        <table className="min-w-full text-sm">
          <thead className="text-slate-300">
            <tr className="border-b border-white/10">
              ...
              <th className="text-left py-2 pr-4">Radius (R⊕)</th>
...
export default function Results() {
  // Replace mockRows with: const rows = useAppStore(s => s.data);
  const rows = mockRows;
  const nav = useNavigate();
  ...
  const stats = useMemo(() => {
    ...
    const c = rows.filter((r) => r.classification === "Candidate").length;
    const fp = rows.filter((r) => r.classification === "False positive").length;
    const ok = rows.filter((r) => r.classification === "Confirmed").length;
    const avgConfidence =
  ...
  const pieData = [
    { name: "Candidates", value: stats.c, color: "#06b6d4" },
    { name: "False Positives", value: stats.fp, color: "#ef4444" },
    { name: "Confirmed", value: stats.ok, color: "#10b981" },
  ];

  const bins = useMemo(() => {
    const b = [
      { r: "0-2", n: 0, label: "Sub-Earth" },
      { r: "2-4", n: 0, label: "Super-Earth" },
      { r: "4-6", n: 0, label: "Mini-Neptune" },
      { r: "6-8", n: 0, label: "Neptune" },
      { r: "8-10", n: 0, label: "Sub-Jupiter" },
      { r: "10+", n: 0, label: "Jupiter+" },
    ];
    for (const x of rows) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white flex items-center justify-center">
        <div className="text-center space-y-6 max-w-md">
          ...
          <h1 className="text-3xl font-bold">No data loaded</h1>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="space-y-2">
            ...
            <p className="text-slate-400">
              Complete analysis of {stats.total} detected objects
            </p>
          </div>
        ...
        <div className="rounded-2xl border border-purple-400/30 bg-gradient-to-br from-purple-500/10 to-pink-500/10 backdrop-blur-sm p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="font-semibold text-purple-200">Model Average Confidence</h3>
              <p className="text-slate-400 text-sm">Overall accuracy of classifications</p>
            </div>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 hover:border-cyan-400/30 transition-all">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-semibold text-cyan-200">Classification Distribution</h3>
                <p className="text-slate-400 text-sm">Proportion of each category</p>
              </div>
            </div>
            <div className="h-64">
              <ResponsiveContainer>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 hover:border-blue-400/30 transition-all">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-semibold text-blue-200">Radius Distribution</h3>
                <p className="text-slate-400 text-sm">Classification by size (R⊕)</p>
              </div>
            </div>
            <div className="h-64">
              <ResponsiveContainer>
                      ...
                      if (active && payload && payload.length) {
                        ...
                        return (
                          <div className="bg-slate-900/95 border border-slate-700 rounded-xl p-3">
                            ...
                            <p className="text-white font-bold">{data.n} objects</p>
                          </div>
                        );
                      }
                      return null;
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 hover:border-purple-400/30 transition-all">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-purple-200">Radius vs Orbital Period</h3>
              <p className="text-slate-400 text-sm">Correlation between size and orbit</p>
            </div>
            <div className="flex gap-4 text-xs">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-slate-300">False Pos.</span>
              </div>
            </div>
          </div>
          <div className="h-80">
            <ResponsiveContainer>
              <ScatterChart>
                ...
                <XAxis
                  ...
                  stroke="#cbd5e1"
                  tick={{ fill: "#cbd5e1" }}
                  label={{ value: "Radius (R⊕)", position: "insideBottom", offset: -5, fill: "#94a3b8" }}
                />
                <YAxis
                  dataKey="period"
                  name="Period"
                  stroke="#cbd5e1"
                  tick={{ fill: "#cbd5e1" }}
                  label={{ value: "Period (days)", angle: -90, position: "insideLeft", fill: "#94a3b8" }}
                />
                <Tooltip
                  contentStyle={{
                    backgroundColor: "rgba(15, 23, 42, 0.95)",
                    ...
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-slate-900/95 border border-slate-700 rounded-xl p-3">
                          ...
                          <p className="text-slate-300">Radius: {data.radius?.toFixed(2)} R⊕</p>
                          <p className="text-slate-300">Period: {data.period?.toFixed(2)} days</p>
                        </div>
                      );
                    }
                    return null;
                ...
                />
                <Scatter
                  data={scatter.filter((s) => s.classification === "Confirmed")}
                  fill="#10b981"
                  fillOpacity={0.7}
                />
                <Scatter
                  data={scatter.filter((s) => s.classification === "Candidate")}
                  fill="#06b6d4"
                  fillOpacity={0.7}
                />
                <Scatter
                  data={scatter.filter((s) => s.classification === "False positive")}
                  fill="#ef4444"
                  fillOpacity={0.5}
                />
              </ScatterChart>
